...
csl_array_t *csl_create_array_impl(csl_dtype_t dtype, const size_t *shape, size_t rank, uint32_t flags)
{
    ...
    {
        if (shape[dim] != 0 && new_array->size > SIZE_MAX / shape[dim])
        {
            fprintf(stderr, "%s:csl_create_array_impl:%u: shape overflow while computing size\n", __FILE__, __LINE__);
            free(new_array->shape);
...
csl_array_t *csl_wrap_array_impl(void *buf_data, csl_dtype_t dtype, const size_t *shape, size_t rank, uint32_t flags)
{
    ...
    if (!buf_data)
    {
        fprintf(stderr, "%s:csl_wrap_array_impl:%u: invalid buf_data parameter (cannot point to a NULL pointer)\n", __FILE__, __LINE__);
        return NULL;
    }
    else if (dtype >= CSL_TYPE_COUNT)
    {
        fprintf(stderr, "%s:csl_wrap_array_impl:%u: invalid dtype parameter\n", __FILE__, __LINE__);
        return NULL;
    }
    else if (!shape)
    {
        fprintf(stderr, "%s:csl_wrap_array_impl:%u: invalid shape parameter\n", __FILE__, __LINE__);
        return NULL;
    }
    else if (rank <= 0)
    {
        fprintf(stderr, "%s:csl_wrap_array_impl:%u: invalid rank parameter\n", __FILE__, __LINE__);
        return NULL;
    }
    else if (flags & CSL_ARRAY_EXTERNAL_DATA)
    {
        fprintf(stdout, "%s:csl_wrap_array_impl:%u: redundant flag triggered\n", __FILE__, __LINE__);
    }
    // Allocating memory for a new array structure.
    csl_array_t *new_array = (csl_array_t *)malloc(sizeof(csl_array_t));
    if (!new_array)
    {
        fprintf(stderr, "%s:csl_wrap_array_impl:%u: failed to allocate memory for a new array structure\n", __FILE__, __LINE__);
        return NULL;
    }
    // Attaching the meta-data to an array.
    new_array->dtype = dtype;
    ...
    if (!new_array->shape)
    {
        fprintf(stderr, "%s:csl_wrap_array_impl:%u: failed to allocate memory for a shape array\n", __FILE__, __LINE__);
        free(new_array);
        return NULL;
    }
    memcpy(new_array->shape, shape, rank * sizeof(size_t));
    ...
    if (!new_array->stride)
    {
        fprintf(stderr, "%s:csl_wrap_array_impl:%u: failed to allocate memory for a stride array\n", __FILE__, __LINE__);
        free(new_array->shape);
        free(new_array);
        return NULL;
    }
    ...
    new_array->offset = 0;
    new_array->owner = NULL;
    new_array->flags = flags | CSL_ARRAY_EXTERNAL_DATA;
    // Calculating the size of the structure.
    new_array->size = 1;
    for (size_t dim = 0; dim < rank; dim++)
    {
        if (shape[dim] != 0 && new_array->size > SIZE_MAX / shape[dim])
        {
            fprintf(stderr, "%s:csl_wrap_array_impl:%u: shape overflow while computing size\n", __FILE__, __LINE__);
            free(new_array->shape);
            free(new_array->stride);
            free(new_array);
            return NULL;
